...
object AuthMiddleware {
  def authenticationMiddleware(service: HttpRoutes[IO]): HttpRoutes[IO] = Kleisli { (req: Request[IO]) =>
    {
  ...
  private def extractToken(req: Request[IO]): IO[Option[String]] =
    IO(req.headers.get[Authorization].collect { case Authorization(Credentials.Token(AuthScheme.Bearer, token)) =>
      token
    })
